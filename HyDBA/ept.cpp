...
extern ULONG64 g_RAM_Size;
extern ULONG64 g_usb_base_addr;
extern ULONG64 g_usb_base_addr2;
////////////////////////////////////////////////////////////////////////////////
//
// types
//
...
_Use_decl_annotations_ EptData *EptInitialization() {
  ...
  //修改PfnBitMap
  //分配PfnBitMap内存，4GB需要4*1024*1024*1024/4096*4 = 256KB， 8GB要512KB， g_RAM_Size/4096*4;
  ept_data->bitmap_size = (ULONG_PTR)(g_RAM_Size / PAGE_SIZE * 4);
  const auto pfn_bitmap = reinterpret_cast<UCHAR *>(ExAllocatePoolWithTag(
	  NonPagedPool, ept_data->bitmap_size, kHyperPlatformCommonPoolTag));
  if (!pfn_bitmap) {
	  ExFreePoolWithTag(ept_pml4, kHyperPlatformCommonPoolTag);
	  ExFreePoolWithTag(ept_poiner, kHyperPlatformCommonPoolTag);
	  ExFreePoolWithTag(ept_data, kHyperPlatformCommonPoolTag);
	  return nullptr;
  }
  RtlZeroMemory(pfn_bitmap, ept_data->bitmap_size);
  ...
  for (auto run_index = 0ul; run_index < pm_ranges->number_of_runs;
    ...
    for (auto page_index = 0ull; page_index < run->page_count; ++page_index) {
      ...
      if (!ept_pt_entry) {
        ...
        ExFreePoolWithTag(ept_poiner, kHyperPlatformCommonPoolTag);
		//修改
		ExFreePoolWithTag(pfn_bitmap, kHyperPlatformCommonPoolTag);
        ExFreePoolWithTag(ept_data, kHyperPlatformCommonPoolTag);
        return nullptr;
      }
    }
  ...
  //修改，把USB 3.0调试端口的地址加入
  for (auto page_index = 0ull; page_index < 0x10; ++page_index) {
	  const auto indexed_addr = g_usb_base_addr + page_index * PAGE_SIZE;
	  const auto ept_pt_entry =
		  EptpConstructTables(ept_pml4, 4, indexed_addr, nullptr);
	  if (!ept_pt_entry) {
		  EptpDestructTables(ept_pml4, 4);
  ...
  //另外一台机子
  for (auto page_index = 0ull; page_index < 0x10; ++page_index) {
	  const auto indexed_addr = g_usb_base_addr2 + page_index * PAGE_SIZE;
	  const auto ept_pt_entry =
		  EptpConstructTables(ept_pml4, 4, indexed_addr, nullptr);
	  if (!ept_pt_entry) {
		  EptpDestructTables(ept_pml4, 4);
  ...
  //修改
  ept_data->pfn_bitmap = pfn_bitmap;
...
_Use_decl_annotations_ static EptCommonEntry *EptpConstructTables(
  ...
  switch (table_level) {
    ...
    case 1: {
	  ...
	  if (ept_pt_entry->all)
	  {
		  //HYPERPLATFORM_LOG_DEBUG("exist. fault_pa: %llx, ept_pt_entry: %x, table %x", physical_address, ept_pt_entry, table);
	  }
...
//修改，简化版，直接创建新的entry. Deal with EPT violation VM-exit 2.
_Use_decl_annotations_ void EptHandleEptViolationExit(EptData *ept_data, ULONG64 fault_pa) 
{
	/*if (!IsReleaseBuild()) {
		...
		{
			HYPERPLATFORM_LOG_DEBUG("[CHECK] EptpIsDeviceMemory pa %llx", fault_pa);
		}
	}*/
	EptpConstructTables(ept_data->ept_pml4, 4, fault_pa, ept_data);
}
...
_Use_decl_annotations_ void EptTermination(EptData *ept_data) {
	//修改
	/*if (ept_data->refer_count--)
	{
		HYPERPLATFORM_LOG_DEBUG("EptTermination ignored.");
		return;
	}*/
  ...
  //修改
  ExFreePoolWithTag(ept_data->pfn_bitmap, kHyperPlatformCommonPoolTag);
  ExFreePoolWithTag(ept_data, kHyperPlatformCommonPoolTag);
}
