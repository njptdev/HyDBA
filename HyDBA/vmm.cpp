...
{
	VmxProcessorBasedControls vmCpuCtlRequested = { (unsigned int)UtilVmRead(VmcsField::kCpuBasedVmExecControl) };
	vmCpuCtlRequested.fields.monitor_trap_flag = State;
	UtilVmWrite(VmcsField::kCpuBasedVmExecControl, vmCpuCtlRequested.all);
}
...
// implementations
//
ULONG GetEptMonitorType(EptData *ept_data, ULONG64 pa)
{
...
PVOID UpdateEptEntryAndBitmap(EptData *ept_data, ULONG64 pa, ULONG ept_value, ULONG monitor_type)
{
	UCHAR *pfn_bitmap = ept_data->pfn_bitmap;
	...
	ept_entry->all = (ept_entry->all & (~0x3ull)) | ept_value;
	pfn_bitmap[byte_offset] = (UCHAR)((pfn_bitmap[byte_offset] & ~(0x3u << bits_offset)) | (monitor_type << bits_offset));
	HYPERPLATFORM_LOG_DEBUG(" Update ept entry %x, pa %llx, ept value %d, monitor type %d->%d",
		ept_entry, pa, ept_value, old_monitor_type, monitor_type);
	return ept_entry;
}
...
_Use_decl_annotations_ static void VmmpHandleVmExit(
  ...
  switch (exit_reason.fields.reason) {
    ...
    case VmxExitReason::kXsetbv:
      ...
      break;
    default:
		HYPERPLATFORM_LOG_DEBUG("Unexpected reason:%x", exit_reason.fields.reason);
...
_Use_decl_annotations_ static void VmmpHandleTripleFault(
  ...
  DbgPrint("guest ip %x, eflags %x, regs %x [di si bp sp, bx dx cx ax], pa %x\n",
	  guest_context->ip, guest_context->flag_reg, guest_context->gp_regs, fault_pa);
...
_Use_decl_annotations_ static void VmmpHandleUnexpectedExit(
  ...
  const auto fault_pa = UtilVmRead(VmcsField::kGuestPhysicalAddress);
  //HYPERPLATFORM_LOG_DEBUG("ip %x, eflags %x, regs %x, pa %x, helper %x",
	//  guest_context->ip, guest_context->flag_reg, guest_context->gp_regs, fault_pa, 
//	  guest_context->stack->processor_data->emu_helper);
  DbgPrint("guest ip %x, eflags %x, regs %x [di si bp sp, bx dx cx ax], pa %x\n",
     guest_context->ip, guest_context->flag_reg, guest_context->gp_regs, fault_pa);
...
_Use_decl_annotations_ static void VmmpHandleMonitorTrap(
    GuestContext *guest_context) {
	HYPERPLATFORM_PERFORMANCE_MEASURE_THIS_SCOPE();
	HYPERPLATFORM_LOG_DEBUG("VmmpHandleMonitorTrap %x", guest_context);
}


void VmmpHandleDebugException(GuestContext *guest_context)
{
	HYPERPLATFORM_LOG_DEBUG("VmmpHandleDebugException ip: %x", guest_context->ip);
	guest_context->flag_reg.fields.tf = false;
  ...
  if (interruption_type == InterruptionType::kHardwareException) {
    ...
    if (vector == InterruptionVector::kPageFaultException) {
      ...
      HYPERPLATFORM_LOG_INFO_SAFE("GuestIp= %016Ix, #GP Code= 0x%2x",
		...
		VmmpHandleDebugException(guest_context);
	}
	else {
...
_Use_decl_annotations_ static void VmmpHandleSetEprocess(
	GuestContext *guest_context, void *context)
{
	HYPERPLATFORM_PERFORMANCE_MEASURE_THIS_SCOPE();
	guest_context->stack->processor_data->shared_data->target_pid = (ULONG)context;

	for (ULONG ept_ind = 1; ept_ind < 2; ept_ind++)
	{
		EptData *ept_data = guest_context->stack->processor_data->shared_data->ept_data_list[ept_ind];
		UCHAR *pfn_bitmap = ept_data->pfn_bitmap;

		for (ULONG64 i = 0; i < ept_data->bitmap_size; i++)
		{
			for (ULONG j = 0; j < 4; j++)
			{
				ULONG64 map2pa = (4 * i + j) << PAGE_SHIFT;
				ULONG monitor_type = (pfn_bitmap[i] >> (j * 2)) & 0x3u;
				if ((monitor_type == 2) || (monitor_type == 3))
				{
					const auto ept_entry = EptGetEptPtEntry(ept_data, map2pa);
					if (!ept_entry)
					{
						HYPERPLATFORM_LOG_DEBUG("[CHECK] ept_entry == NULL, pa %x", map2pa);
					}
					//ept_entry->fields.write_access = 0;  //E+R£¬101
					HYPERPLATFORM_LOG_DEBUG("PD/PT type %d, ept entry %x, pa %llx", monitor_type, ept_entry, map2pa);
				}
...
_Use_decl_annotations_ static void VmmpHandleInvalidEpt(GuestContext *guest_context, void *context)
{
	UtilInveptGlobal();
	UtilInvvpidAllContext();
	//HYPERPLATFORM_COMMON_BUG_CHECK(HyperPlatformBugCheck::kUnspecified, (ULONG)context, 0, 0);
}

// EXIT_REASON_EPT_VIOLATION
_Use_decl_annotations_ static void VmmpHandleEptViolationVmCall(
	GuestContext *guest_context, ULONG64 fault_pa) {
	HYPERPLATFORM_PERFORMANCE_MEASURE_THIS_SCOPE();
	auto processor_data = guest_context->stack->processor_data;
	EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_MONITOR1], fault_pa);
	EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_NORMAL], fault_pa);
	EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_ANALYSIS], fault_pa);
	UtilInveptGlobal();
	UtilInvvpidAllContext();
}
  ...
  switch (hypercall_number) {
    ...
    case HypercallNumber::kGetSharedProcessorData:
      *reinterpret_cast<void **>(context) =
      ...
      VmmpIndicateSuccessfulVmcall(guest_context);
      break;
	case HypercallNumber::kSetEprocess:
		VmmpHandleSetEprocess(guest_context, context);
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	case HypercallNumber::kUpdateEptMonitor:
		VmmpHandleSetEprocess(guest_context, context);
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	case HypercallNumber::kHandleEptVoilation:
		fault_pa = *(ULONG64 *)context;
		VmmpHandleEptViolationVmCall(guest_context, fault_pa);
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	case HypercallNumber::kClearMonitorFlag:
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	case HypercallNumber::kGetProcessorData:
		*reinterpret_cast<void **>(context) = guest_context->stack->processor_data;
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	case HypercallNumber::kInvalidEpt:
		VmmpHandleInvalidEpt(guest_context, context);
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
    default:
...
_Use_decl_annotations_ static void VmmpHandleEptViolation(
  ...
  //HYPERPLATFORM_LOG_DEBUG("Warning: VmmpHandleEptViolation fault_pa %llx, ip %x", fault_pa, guest_context->ip);
  EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_MONITOR1], fault_pa);
  EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_NORMAL], fault_pa);
  EptHandleEptViolationExit(processor_data->shared_data->ept_data_list[EPTP_ANALYSIS], fault_pa);
  UtilInveptGlobal();
...
_Use_decl_annotations_ static void VmmpHandleEptMisconfig(
  ...
  const auto fault_va = UtilVmRead(VmcsField::kGuestLinearAddress);
  ...
  const auto ept_pt_entry1 = EptGetEptPtEntry(
	  guest_context->stack->processor_data->shared_data->ept_data_list[EPTP_NORMAL], fault_address);
  const auto ept_pt_entry2 = EptGetEptPtEntry(
	  guest_context->stack->processor_data->shared_data->ept_data_list[EPTP_MONITOR1], fault_address);
  HYPERPLATFORM_LOG_DEBUG("ip %x, eflags %x, regs %x, va %x, pa %x, helper %x, entry %x %x",
	  guest_context->ip, guest_context->flag_reg, guest_context->gp_regs, fault_va, fault_address,
	  guest_context->stack->processor_data->emu_helper, ept_pt_entry1, ept_pt_entry2);
  HYPERPLATFORM_COMMON_BUG_CHECK(HyperPlatformBugCheck::kEptMisconfigVmExit,
                                 fault_address,
                                 reinterpret_cast<ULONG_PTR>(ept_pt_entry2), 0);
}
